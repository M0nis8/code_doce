...
def view_data():
    passwd_wind = ctk.CTkToplevel(root)
    passwd_wind.title("Enter Password")
    passwd_wind.geometry("352x150")  

    password_label = ctk.CTkLabel(passwd_wind, text="Password:")
    ...
    passwd_wind.grab_set()
    passwd_wind.transient(root)
    passwd_wind.mainloop()

def display_data():
    disp_data = ctk.CTkToplevel(root)
    disp_data.title("User Data")
    disp_data.geometry("600x400")  
